...
Trit_num Tritset::Reference::operator=(const Trit_num& trit) {
	...
	auto void_mask = static_cast<uint>(~(three << shift));
...
Trit_num Tritset::Reference::operator=(const Reference& trit_ref) {
	Trit_num trit = static_cast<Trit_num>(three & (trit_ref.tritset->set[array_pos] >> ((BYTE_SIZE * sizeof(uint) / TRIT_SIZE - (int_pos + 1)) * TRIT_SIZE)));
...
Trit_num Tritset::Reference::operator~()const {
	...
	uint get_mask = static_cast<uint>(three << shift);

	uint trit = (tritset->set[array_pos] & get_mask) >> shift;
	if (trit == Unknown) return Unknown;
	Trit_num untrit = static_cast<Trit_num>(three & ~trit);
